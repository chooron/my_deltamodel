...
class FlexMopexV1(nn.Module):
    ...
    def __init__(
        ...
        # 总参数数量：模型参数 * nmul + 路由参数 + 权重参数 * 2(On/Off) * nmul
        ...
        self.learnable_param_count = (
            ...
            + len(self.weight_names) * 2 * self.nmul  # 注意：乘以2
        )
    ...
    def _descale_routing_params(
        ...
        return {
            name: change_param_range(
                params[:, i], self.ROUTING_BOUNDS[name]
            )
    ...
    def unpack_parameters(
        ...
        Args:
            parameters: 字典，包含 MOPEX 参数、权重和路由参数
                例如: {"MOPEX": [B, 12*nmul], "WEIGHTS": [B, 8*nmul], 
                      "GAMMA_UH": [B, 2]}
                注意：WEIGHTS 维度是 8*nmul，因为每个过程有2个logits (Off/On)

        Returns:
            - mopex_params: MOPEX 模型参数 [B, 12, nmul]
            - weights: 结构权重 [B, 4, nmul] (每个过程独立经过gumbel_softmax)
        ...
        # 提取并激活结构权重（使用独立的gumbel_softmax）
        raw_weights = parameters["weights"]  # [B, 8*nmul]
        
        # Reshape: [B, 8*nmul] -> [B, 4, 2, nmul]
        # 4个过程，每个过程2个logits (Off, On)
        weights_reshaped = raw_weights.view(
            raw_weights.shape[0], len(self.weight_names), 2, self.nmul
        )  # [B, 4, 2, nmul]
        
        # 转置为 [B, nmul, 4, 2]，方便批量处理
        weights_transposed = weights_reshaped.permute(0, 3, 1, 2)  # [B, nmul, 4, 2]
        
        # 对每个过程在每个nmul下独立应用gumbel_softmax（在最后一个维度dim=-1，即2个状态上）
        # 这样4个过程互不影响，每个都做独立的 Off/On 决策
        if self.training:
            weights_probs = F.gumbel_softmax(
                weights_transposed, tau=1.0, hard=False, dim=-1
            )  # [B, nmul, 4, 2]
        else:
            weights_probs = F.softmax(weights_transposed, dim=-1)
        
        # 提取 "On" 状态的概率（索引1），形状 [B, nmul, 4]
        weights_on = weights_probs[..., 1]  # [B, nmul, 4]
        
        # 转回 [B, 4, nmul]
        weights = weights_on.permute(0, 2, 1)  # [B, 4, nmul]
        
        # 提取并激活路由参数
        routing_params = self.activate(parameters["gamma_uh"])
        
        return mopex_params, weights, routing_params
    ...
    def forward(
        ...
        # 解包参数
        mopex_params_raw, weights_raw, routing_raw = self.unpack_parameters(parameters)
