"""
Blend hydrological model (V3 - Snow State Enhanced)

相比V2的核心改进：
1. **雪层状态感知**：
   - 针对有融雪模块的模型（HBV, EXPHYDRO），额外提取雪层覆盖状态
   - 雪层状态归一��后与土壤湿度状态一起作为门控网络输入
   - 增强门控网络对季节性积雪过程的感知能力
...
Architecture:
    ...
    Physics Layer (4 Models in Unified Loop)
      ↓
    [Q₁, Q₂, Q₃, Q₄] + [Soil_States] + [Snow_States]
      ↓
    Gating Network (LSTM) - 输入包含土壤和雪层状态
      ↓
    Weights [w₁, w₂, w₃, w₄]
      ↓
    Q_blend = Σ(wᵢ × Qᵢ)
...
Models with Snow Module:
    - HBV: S1 (snow storage)
    - EXPHYDRO: snow_storage
    - SHM: No snow module (填充0)
    - HYMOD: No snow module (填充0)
...
class BlendHydroV3(nn.Module):
    """优化版Blend水文模型：统一时间循环 + 纯静态参数 + torch.compile + 雪层状态感知"""
    ...
    def __init__(
        ...
        # 初始化LSTM门控网络
        # 输入维度：3(气象) + Nmul*NumModels*2(土壤状态+雪层状态) + static_attr_dim(属性)
        # 输出维度：NumModels（只预测模型级权重）
        gating_input_dim = (
            3 + self.nmul * len(self.model_order) * 2 + self.num_attributes
        )
        print(
    ...
    def _normalize_snow_states(
        ...
        """
        norm_snow_states = torch.zeros_like(raw_snow_states)

        # HBV: S1 (snow storage) - 使用一个合理的最大值进行归一化
        # 由于雪层没有明确的容量参数，使用经验值或动态最大值
        if "HBV" in self.model_order:
            idx = self.model_order.index("HBV")
            # 使用动态归一化：除以时间序列中的最大值 + 一个小的常数
            max_snow = raw_snow_states[:, :, :, idx].max(dim=0, keepdim=True)[0]
            norm_snow_states[:, :, :, idx] = raw_snow_states[:, :, :, idx] / (
                max_snow + self.nearzero + 100.0  # 添加100mm作为基准
            )

        # EXPHYDRO: snow_storage - 同样使用动态归一化
        if "EXPHYDRO" in self.model_order:
            idx = self.model_order.index("EXPHYDRO")
            max_snow = raw_snow_states[:, :, :, idx].max(dim=0, keepdim=True)[0]
            norm_snow_states[:, :, :, idx] = raw_snow_states[:, :, :, idx] / (
                max_snow + self.nearzero + 100.0
            )
        ...
        # 裁剪到合理范围
        norm_snow_states = torch.clamp(norm_snow_states, 0.0, 2.0)

        return norm_snow_states
    ...
    def _unified_timestep_loop(
        ...
        Returns:
            outputs: 各模型的流量输出字典
            soil_states_seq: 各模型的核心土壤状态序列 [n_steps, n_grid, nmul, num_models]
            snow_states_seq: 各模型的雪层状态序列 [n_steps, n_grid, nmul, num_models]
        """
        # 初始化状态
        ...
        for t in range(n_steps):
            ...
            if "HBV" in self.model_order:
                ...
                # 记录雪层状态：S1 (Snow Storage)
                snow_states_seq[t, :, :, idx] = S1
            ...
            if "EXPHYDRO" in self.model_order:
                ...
                # 记录雪层状态：snow_storage
                snow_states_seq[t, :, :, idx] = snow_storage
            ...
            if "HYMOD" in self.model_order:
                ...
                # HYMOD无雪层模块，snow_states_seq保持为0

        return outputs, soil_states_seq, snow_states_seq
    ...
    def forward(
        ...
        # raw_soil_states: [Time, Grid, Nmul, NumModels]
        # raw_snow_states: [Time, Grid, Nmul, NumModels]
        model_outputs, raw_soil_states_seq, raw_snow_states_seq = self._unified_timestep_loop(
        ...
        # 5.1 处理土壤状态数据：展平 Nmul 和 Model 维度，作为特征
        # [Time, Grid, Nmul, NumModels] -> [Time, Grid, Nmul * NumModels]
        norm_soil_states = self._normalize_states(raw_soil_states_seq, params_dict)
        flat_soil_states_feat = norm_soil_states.view(n_steps, n_grid, -1)

        # 5.2 处理雪层状态数据：展平 Nmul 和 Model 维度，作为特征
        # [Time, Grid, Nmul, NumModels] -> [Time, Grid, Nmul * NumModels]
        norm_snow_states = self._normalize_snow_states(raw_snow_states_seq, params_dict)
        flat_snow_states_feat = norm_snow_states.view(n_steps, n_grid, -1)

        # 5.3 拼接动态输入：气象 + 土壤状态 + 雪层状态
        # x_nn_norm: [Time, Grid, 3]
        # x_dynamic: [Time, Grid, 3 + (Nmul*NumModels) + (Nmul*NumModels)]
        # 这里 LSTM 同时看到了所有 Parameter Set 的土壤和雪层状态
        x_dynamic = torch.cat([x_nn_norm, flat_soil_states_feat, flat_snow_states_feat], dim=-1)
