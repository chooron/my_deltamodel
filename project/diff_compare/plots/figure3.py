...
from dmg.models.hydromodel import PARAM_INFO, NUMBER_INFO  # noqa
...
# baseline = Marrmot, ours = DIF
COLOR_BASE = '#D64045'  # Marrmot
COLOR_OURS = '#2274A5'  # DIF
MARKER_BASE = '^'
MARKER_OURS = 'o'
...
def plot_enhanced_generalization(df_stats):
    ...
    best_gen = df_stats.nsmallest(2, 'gap_dif')
    ...
    ax1.set_xlabel('Training Performance ($KGE_{train}$)')
    ax1.set_ylabel('Testing Performance ($KGE_{test}$)')
    ...
    df_melt['method'] = df_melt['method'].map({'gap_mar_med': 'Baseline', 'gap_dif_med': 'Ours'})
    ...
    sns.stripplot(
        ...
        hue='method',
        ax=ax2,
        palette={'Baseline': '#9b2f2f', 'Ours': '#1c5a7a'},
        dodge=True,
        jitter=True,
        size=4,
        alpha=0.4,
    ...
    ax2.set_ylabel('Generalization Gap ($KGE_{train} - KGE_{test}$)')
    ...
    ax2.legend(handles=handles[:2], labels=['Baseline (Marrmot)', 'Ours (DIF)'], loc='upper left')
