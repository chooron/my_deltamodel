...
def safe_pow(base: torch.Tensor, exp: torch.Tensor, min_base: float = 0.01) -> torch.Tensor:
    """x^n 的梯度安全版本。
    当 base→0 且 exp 为分数时，grad = n*x^(n-1) → inf，导致 NaN。
    将 base 钳位到 min_base 以保证梯度有界。"""
    return base.clamp(min=min_base).pow(exp)
...
def rainsnow_dingman(P, T, ts):
    ...
    sf = torch.clamp(0.5 * (1.0
        + torch.exp(-2.2 * safe_pow(dw + 1e-6, 1.3))
        - torch.exp(-2.2 * safe_pow(dc + 1e-6, 1.3))), 0.0, 1.0)
    return P * sf, P * (1.0 - sf)
...
def snobal_hmets(snowfall, rainfall, T, S_snow, S_liq, S_cum,
    ...
    melt = torch.minimum(S_snow, ddf * F.relu(T - tt_melt))
    refreeze = torch.minimum(S_liq, kf * safe_pow(F.relu(tt_refreeze - T) + eps, refreeze_exp))
...
def soilevap_vic(PET, c, S, Smax, gamma, eps=1e-6):
    ratio = torch.clamp(S / (Smax + eps), 0.0, 1.0)
    return torch.minimum(PET * c * (1.0 - safe_pow(1.0 - ratio, gamma)), S)
...
def quick_vic(S, Smax, k, n, eps=1e-6):
    """VIC style: 幂律调制 + 下界保护.
    modulation = 0.3 + 0.7 * ratio^n
    ratio→1 时退化为 linear; ratio→0 时最低保留 30% 的 k."""
    ratio = (S / (Smax + eps)).clamp(0.0, 1.0)
    mod = 0.3 + 0.7 * safe_pow(ratio, n)
    return S * (1.0 - torch.exp(-k * mod))

def quick_topmodel(S, Smax, k, lam, eps=1e-6):
    """TOPMODEL style: 指数亏缺衰减 + 下界保护.
    modulation = 0.3 + 0.7 * exp(-lam*(1-ratio))
    ratio→1 时退化为 linear; 亏缺越大排泄指数下降, 但最低保留 30% 的 k."""
    ratio = torch.clamp(S / (Smax + eps), 0.0, 1.0)
    mod = 0.3 + 0.7 * torch.exp(-lam * (1.0 - ratio))
    return S * (1.0 - torch.exp(-k * mod))
...
def base_power(S, k, n, Smax, eps=1e-6):
    """Power-law reservoir: effective rate depends on storage level.
    k_eff = k * (S/Smax)^(n-1)  →  output ∝ S, bounded by S.
    n>1: drainage rate increases with fullness (deep aquifer).
    n<1: drainage rate decreases with fullness (macropore)."""
    ratio = (S / (Smax + eps)).clamp(eps, 1.0)
    return S * (1.0 - torch.exp(-k * safe_pow(ratio, n - 1.0)))
...
def diff_blend_step(
    ...
    """单时间步，返回 (Q_total, states..., balance_opts)。"""
    ...
    o3, Ss3, Sl3, Sc3 = snobal_hmets(snowfall, rainfall, T_t, S_snow, S_liq, S_cum,
                                      ...
                                      p["x15"], p["x27"], eps)
    snow_out_stack = torch.stack([o1, o2, o3], -1)  # [B, nmul, 3]
    snow_out = (snow_out_stack * w_sn).sum(-1)
    ...
    inf_stack = torch.stack([i1, i2, i3], -1)  # [B, nmul, 3]
    ...
    ev_stack = torch.stack([e1, e2, e3], -1)  # [B, nmul, 3]
    ...
    q2 = quick_vic(S_top, p["Smax_top"], p["k_quick"], p["x6"], eps)
    q3 = quick_topmodel(S_top, p["Smax_top"], p["k_quick"], p["x7"], eps)
    qf_stack = torch.stack([q1, q2, q3], -1)  # [B, nmul, 3]
    ...
    b2 = base_power(S_phreatic, p["k_base"], p["x12"], p["Smax_ph"], eps)
    bf_stack = torch.stack([b1, b2], -1)  # [B, nmul, 2]
    ...
    # 收集balance选项 (平均nmul维度，保留梯度)
    balance_opts = {
        "snow_outflow": snow_out_stack.mean(dim=1),  # [B, 3]
        "infiltration": inf_stack.mean(dim=1),       # [B, 3]
        "evaporation": ev_stack.mean(dim=1),         # [B, 3]
        "quickflow": qf_stack.mean(dim=1),           # [B, 3]
        "baseflow": bf_stack.mean(dim=1),            # [B, 2]
    }

    return Q_total, S_snow, S_liq, S_cum, S_top, S_phreatic, balance_opts
...
class DiffBlendV1(nn.Module):
    ...
    def forward(self, x_dict: Dict[str, torch.Tensor],
        ...
        for t in range(n_steps):
            ...
            # 收集 balance buffer (保留梯度链)
            for proc_name, val in balance_opts.items():
                balance_buf_lists[proc_name].append(val)  # [B, n_options]
        ...
        # --- 公式累积平衡约束序列 (warmup 截断前) ---
        # 计算每个过程不同公式的平均值，然后计算每个公式与平均值的相对差值
        for proc_name, buf in balance_stacked.items():
            rolled = self._rolling_sum(buf, self.balance_window)  # [T, B, n_options]
            # 计算所有公式的平均值
            mean_val = rolled.mean(dim=-1, keepdim=True)  # [T, B, 1]
            # 用 sqrt(var + eps) 代替 std + eps，避免 std→0 时反向传播梯度爆炸
            # detach scale：归一化尺度不参与梯度，仅用于让 loss 尺度无关
            # 使用 mean(abs) 作为备选尺度，当 var 很小时提供更稳定的归一化
            ...
            # 计算每个公式与平均值的归一化差值 (差值/尺度)
            n_opt = buf.shape[-1]
            for j in range(n_opt):
                # 使用标准差归一化，而不是除以平均值，更稳定
                normalized_diff = (rolled[:, :, j] - mean_val.squeeze(-1)) / scale.squeeze(-1)
                # Clamp 防止极端值（收紧范围，降低梯度爆炸风险）
