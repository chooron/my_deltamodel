...
class RNNModel(nn.Module):
    """
    一个通用的RNN模型，可以通过字符串选择RNN类型。

    参数:
        input_size (int): 输入特征的数量。
        hidden_size (int): RNN隐藏层的大小。
        rnn_type (str): 'LSTM', 'GRU', 或 'RNN'。
        num_layers (int): RNN堆叠的层数。
        dropout (float): RNN层之间的dropout比率。
    """

    def __init__(self, input_size, hidden_size=64, rnn_type='LSTM', num_layers=1, dropout=0.0):
        super().__init__()
        ...
        # 输出层，将RNN的最后一个时间步的输出映射到预测值
        self.fc = nn.Linear(hidden_size, 1)

    def forward(self, x):
        # x 形状: (batch_size, sequence_length, input_size)
        # RNN/LSTM/GRU forward pass
        # out 形状: (batch_size, sequence_length, hidden_size)
        # hidden/cell 形状: (num_layers, batch_size, hidden_size)
        out, _ = self.rnn(x)
...
class LitRNN(pl.LightningModule):
    ...
    def __init__(self, input_size, hidden_size=64, dropout=0.2, rnn_type='LSTM',
...
if __name__ == '__main__':
    ...
    model_checkpoint = ModelCheckpoint(
        dirpath='checkpoints',
        filename=f'{RNN_TYPE}-best-{{epoch:02d}}-{{val_r2:.4f}}',
        save_top_k=1,
        monitor='val_r2',  # 监控验证集 R2 分数
        mode='max'  # 模式是 'max'
    )
