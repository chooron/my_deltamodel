...
def plot_parameter_distributions(
    ...
    if basin_idx >= n_basins:
        raise ValueError(f"Basin index {basin_idx} out of range [0, {n_basins-1}]")
    ...
    fig.suptitle(f'Parameter Distributions - Basin {basin_idx} ({dataset.upper()} Dataset)',
    ...
    for i in range(n_params):
        ...
        # 添加统计线
        ylim = ax.get_ylim()
        ax.axvline(basin_mean[i], color='red', linestyle='--', linewidth=2,
                   label=f'Mean: {basin_mean[i]:.4f}')
        ax.axvline(basin_p10[i], color='green', linestyle=':', linewidth=1.5,
                   label=f'P10: {basin_p10[i]:.4f}')
        ax.axvline(basin_p90[i], color='orange', linestyle=':', linewidth=1.5,
...
# }
    ...
    # 加载数据
    params_samples, params_stats = load_mc_dropout_parameters(mc_dropout_dir, dataset)
    n_samples, n_basins, n_params = params_samples.shape

    print(f"Loaded {n_samples} samples for {n_basins} basins with {n_params} parameters")

    # 调整参数名称列表长度
    param_names = param_names[:n_params]

    # 绘制分布图
    save_path = os.path.join(figures_dir, f"param_distributions_basin{basin_idx}_{dataset}.png")
    plot_parameter_distributions(
        params_samples, params_stats, basin_idx,
        param_names=param_names, save_path=save_path, dataset=dataset
    )

    print("\nParameter statistics for basin", basin_idx)
    print("-" * 60)
    for i, name in enumerate(param_names):
        print(f"{name:12s}: Mean={params_stats['mean'][basin_idx, i]:.4f}, "
