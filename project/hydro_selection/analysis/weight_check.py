...
parser = argparse.ArgumentParser(
    description="查看某个流域的模型权重随时间的变化"
)
parser.add_argument(
    "--basin", type=int, default=9, help="流域索引（0-based）"
)
parser.add_argument(
    "--save_dir", type=str, default=".", help="保存可视化图像的目录"
)
...
if basin_idx < 0 or basin_idx >= weights.shape[1]:
    raise ValueError(
        f"无效的流域索引: {basin_idx}，应在 [0, {weights.shape[1] - 1}] 范围内"
    )
...
for i, name in enumerate(model_names):
    print(
        f"  {name}: {basin_weights[:, i].mean():.4f} (std: {basin_weights[:, i].std():.4f})"
    )
...
ax1.set_title(
    f"Basin {basin_idx}: Model Weights Over Time (Stacked Area)",
    fontsize=14,
    fontweight="bold",
)
...
fig1.tight_layout()
path1 = os.path.join(
    args.save_dir, f"basin_{basin_idx}_model_weights_stack.png"
)
...
for i, (name, color) in enumerate(zip(model_names, colors)):
    ax2.plot(
        t, basin_weights[:, i], label=name, color=color, linewidth=2, alpha=0.8
    )
ax2.set_title(
    f"Basin {basin_idx}: Model Weights Over Time (Line Chart)",
    fontsize=14,
    fontweight="bold",
)
...
fig2.tight_layout()
path2 = os.path.join(
    args.save_dir, f"basin_{basin_idx}_model_weights_lines.png"
)
...
bp = ax3.boxplot(
    [basin_weights[:, i] for i in range(4)],
    labels=model_names,
    patch_artist=True,
    showmeans=True,
...
ax3.set_title(
    f"Basin {basin_idx}: Model Weight Distribution",
    fontsize=14,
    fontweight="bold",
)
...
fig3.tight_layout()
path3 = os.path.join(
    args.save_dir, f"basin_{basin_idx}_model_weights_boxplot.png"
)
...
ax4.set_title(
    f"Basin {basin_idx}: Model Weights Heatmap", fontsize=14, fontweight="bold"
)
cbar = plt.colorbar(im, ax=ax4)
cbar.set_label("Weight Value", fontsize=11)
fig4.tight_layout()
path4 = os.path.join(
    args.save_dir, f"basin_{basin_idx}_model_weights_heatmap.png"
)
...
ax5.set_title(
    f"Basin {basin_idx}: Average Model Weight Distribution",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
