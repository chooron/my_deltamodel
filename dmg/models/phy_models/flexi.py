...
_flexi_production_step = torch.compile(
    _flexi_production_step_impl
)
_flexi_routing_step = torch.compile(
    _flexi_routing_step_impl
)
...
class Flexi(UnifyV2):
    ...
    def _run_model(
        ...
        for t in range(n_steps):
            flux_rf, flux_rs_total, flux_ea, S1, S2 = _flexi_production_step(
                P_seq[t],
                PET_seq[t],
                S1,
                S2,
        ...
        for t in range(n_steps):
            Qsim, S3, S4 = _flexi_routing_step(
