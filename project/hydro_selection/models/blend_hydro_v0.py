...
class BlendHydroV0(nn.Module):
    ...
    def _set_parameters(self) -> None:
        ...
        total_routing_params = len(self.routing_param_names) * len(self.model_order)
        self.learnable_param_count = total_params * self.nmul + total_routing_params
    ...
    def unpack_parameters(
        ...
        """
        # 提取所有模型的路由参数 (应该是 [B, num_models*2])
    ...
    def forward(
        ...
        # 对每个模型分别应用gamma单位线汇流（使用各自的路由参数）
        for name, output in model_outputs.items():
            # 平均nmul维度得到每个模型的产流结果
            q_prerouting = output.mean(-1)
            # 也保存路由前的结果
            result[f"{name.lower()}_prerouting"] = q_prerouting
            # 对该模型应用gamma汇流（传入模型名称以使用对应的路由参数）
            q_routed = self._apply_routing(q_prerouting, name, n_steps, n_grid)
            # 存储路由后的结果
            result[f"{name.lower()}_streamflow"] = q_routed

        # 计算所有模型streamflow的平均作为blend输出（可选）
        all_streamflows = torch.stack(
            [result[f"{name.lower()}_streamflow"] for name in self.model_order],
            dim=-1
        )
        result["streamflow"] = all_streamflows.mean(-1)
        result["target"] = x_dict["target"]
