...
from dmg.models.hydromodel import PARAM_INFO # noqa
...
def load_and_process_data():
    ...
    for group_name, model_list in groups.items():
        ...
        if "log" in metrics_map:
            data_ready[group_name]["log"] = metrics_map["log"][model_list].mean(
                axis=1
            )
...
def _plot_ranked_curve(ax, data_norm, data_log, show_xlabel=False):
    """Plot ranked improvement curves (sorted deltas) for normal/log flows."""
    ...
    ax.plot(x_norm, y_norm, color="black", lw=1.3, label="KGE(Q)", zorder=3)
    ax.plot(x_log, y_log, color="0.35", lw=1.2, ls="--", label="KGE(1/Q)", zorder=2)
    ...
    ax.fill_between(x_log, 0, y_log, where=y_log >= 0, color=COLOR_GAIN, alpha=0.16)
    ax.fill_between(x_log, 0, y_log, where=y_log < 0, color=COLOR_LOSS, alpha=0.16)
    ...
    if show_xlabel:
        ...
        ax.set_xticklabels([])
    ax.set_ylabel(r"Sorted Gain ($\Delta$KGE(Q), $\Delta$KGE(1/Q))")
...
def plot_spatial_statistical_composite(gdf, data_structure):
    ...
    row_titles = [
        ...
        "Simple Models (<5 Params)",
        "Complex Models (>8 Params)",
    ]
    ...
    for i, group in enumerate(rows):
        ...
        _plot_ranked_curve(ax_stat, s_norm, s_log, show_xlabel=(i == len(rows) - 1))

        if i == 0:
            ax_stat.set_title(
                "(c) Gain Distribution Profile", fontsize=11, fontweight="bold"
            )
            ax_stat.legend(loc="lower right", fontsize=8, framealpha=0.9)
    ...
    cbar.ax.set_xticklabels(
        ["baseline (marrmot) better", "neutral", "ours (dif) better"]
    )
    ...
    Red markers indicate basins where Diff-MARRMoT outperforms the baseline (Delta > 0).
