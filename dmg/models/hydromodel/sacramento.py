...
SACRAMENTO_PARAMS_BOUNDS = {
    ...
    "pfree": [0.0, 1.0],      # Fraction of percolation directed to free water stores [-]
...
def sacramento_step(
    ...
    uztwm = f1 * smax
    uzfwm = torch.maximum(torch.tensor(0.005 / 4.0, device=P.device), f2 * (smax - uztwm))
    lztwm = torch.maximum(torch.tensor(0.005 / 4.0, device=P.device), f3 * (smax - uztwm - uzfwm))
    lzfwpm = torch.maximum(torch.tensor(0.005 / 4.0, device=P.device), f4 * (smax - uztwm - uzfwm - lztwm))
    lzfwsm = torch.maximum(torch.tensor(0.005 / 4.0, device=P.device), (1.0 - f4) * (smax - uztwm - uzfwm - lztwm))
    ...
    flux_twexu = torch.clamp(flux_twexu, min=0.0, max=flux_peff) # Bound constraint
    ...
    flux_euztw = torch.minimum(flux_euztw, S1_tmp - nearzero) # Prevent negative state
    
    S1_new = torch.clamp(S1_tmp - flux_euztw, min=nearzero)

    # Inflow to S2 from S1 excess
    flux_qsur = saturation_1(flux_twexu, S2, uzfwm)
    flux_qsur = torch.clamp(flux_qsur, min=0.0, max=flux_twexu)
    ...
    flux_ru = soilmoisture_1(S1_new, uztwm, S2_tmp_int, uzfwm, nearzero=nearzero)
    ...
    flux_pc = percolation_4(pbase, zperc, rexp, total_def, total_lmask, S2_tmp_ru, uzfwm, nearzero=nearzero)
    ...
    S3_new_tmp = torch.clamp(S3_tmp_evap + flux_pctw - flux_twexl - flux_elztw, min=nearzero)
    ...
    dist_p, dist_s = deficit_based_distribution(S4, lzfwpm, S5, lzfwsm, nearzero=nearzero)
    ...
    flux_rlp = soilmoisture_2(S3_new_tmp, lztwm, S4, lzfwpm, S5, lzfwsm, nearzero=nearzero)
    flux_rls = soilmoisture_2(S3_new_tmp, lztwm, S5, lzfwsm, S4, lzfwpm, nearzero=nearzero)

    # Finalize state updates
    S3_new = torch.clamp(S3_new_tmp + flux_rlp + flux_rls, min=nearzero)
    
    S4_new = torch.clamp(S4 + flux_twexlp + flux_pcfwp - flux_rlp - flux_qbfp, min=nearzero)
    
    S5_new = torch.clamp(S5 + flux_twexls + flux_pcfws - flux_rls - flux_qbfs, min=nearzero)
