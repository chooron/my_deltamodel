...
class WeightMLPGate(nn.Module):
    """MLP门控网络用于基于内部状态、气象输入和静态属性预测动态权重 (Sigmoid版本)"""
    ...
    def __init__(
        ...
        # 输出层：直接预测4个权重的logits (sigmoid版本)
        self.fc_out = nn.Linear(hidden_size, num_weights)
    ...
    def forward(
        ...
        Returns:
            weights_logits: [n_grid, 4] 权重logits
        """
        # 拼接输入
        ...
        h = F.relu(self.fc2(h))
        h = self.dropout2(h)
        weights_logits = self.fc_out(h)  # [n_grid, 4]

        return weights_logits


class FlexMopexV31(nn.Module):
    """MOPEX 水文模型：MLP Gate动态权重 + 物理参数 + torch.compile (Sigmoid版本)

    版本三_1：MLP Gate 基于内部状态预测动态权重，使用sigmoid激活函数
    ...
    def __init__(
        ...
        self.name = "FlexMopexV3_1MLPGate"
    ...
    def _predict_weights_from_states(
        ...
        # 应用sigmoid激活函数
        weights_on = torch.sigmoid(weights_logits)  # [n_grid, 4]
