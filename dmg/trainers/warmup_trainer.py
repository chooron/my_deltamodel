...
class WarmupTrainer(BaseTrainer):
    ...
    def _calc_multi_streamflow_loss(
        self,
        model_output: dict,
        dataset_sample: dict,
    ) -> torch.Tensor:
        """计算多个模型streamflow输出的平均loss。
        
        Parameters
        ----------
        model_output
            模型输出字典，包含多个{model_name}_streamflow
        dataset_sample
            数据样本，包含target
        ...
        # 获取所有streamflow输出的key
        streamflow_keys = [k for k in model_output.keys() if k.endswith("_streamflow")]
        
        # 对每个模型的streamflow计算loss
        for key in streamflow_keys:
            pred = model_output[key]
            # 使用loss_func计算loss，传递sample_ids参数
            loss_i = self.loss_func(
                pred, 
                target,
                sample_ids=dataset_sample['batch_sample'],
            )
        ...
        if hasattr(self.model, "loss_dict"):
            self.model.loss_dict["multi_streamflow_loss"] = avg_loss.item()
            for i, key in enumerate(streamflow_keys):
                self.model.loss_dict[f"{key}_loss"] = losses[i].item()
    ...
    def train_one_epoch(
        ...
        for mb in tqdm.tqdm(
            ...
            # 计算多个streamflow的loss并平均
            loss = self._calc_multi_streamflow_loss(model_output, dataset_sample)
            loss.backward()
    ...
    def calc_metrics(
        ...
        # 获取所有streamflow keys
        streamflow_keys = [k for k in batch_predictions[0].keys() 
                          if k.endswith("_streamflow")]
        
        # 对每个streamflow分别计算指标
        for key in streamflow_keys:
            predictions = self._batch_data(batch_predictions, key)
            target_trimmed = target[: len(predictions), :]
            
            # Compute metrics
            metrics = Metrics(
                np.swapaxes(predictions.squeeze(), 1, 0),
                np.swapaxes(target_trimmed.squeeze(), 1, 0),
            )
            
            # Save metrics with model-specific prefix
            output_path = os.path.join(self.config["out_path"], key.replace("_streamflow", ""))
            os.makedirs(output_path, exist_ok=True)
            metrics.dump_metrics(output_path)
            
            log.info(f"Metrics for {key} saved to {output_path}")
