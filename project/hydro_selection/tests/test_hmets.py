...
import sys
sys.path.append('E:\\PaperCode\\generic_deltamodel')
from project.models.hmets import HMETS
...
if __name__ == '__main__':
    ...
    n_steps = 365*3  # 模拟总时长 (天)
    n_grid = 3         # 模拟的流域/网格数量 (可以设为多个)
    ...
    # a. 生成单个气象变量的时间序列
    t_cycle = torch.linspace(0, 2 * torch.pi * (n_steps / 365), n_steps, device=device)
    Tmean = 10 * torch.sin(t_cycle - torch.pi / 2) + 5
    Tmin = Tmean - 5
    Tmax = Tmean + 5
    Prcp = torch.clamp(torch.randn(n_steps, device=device) * 5 + 2, min=0)
    Pet = torch.clamp(torch.randn(n_steps, device=device) * 1, min=0)

    # b. 按 hmet_model.variables 定义的顺序堆叠变量
    # hmet_model.variables -> ['Tmin', 'Tmax', 'Prcp', 'Pet']
    x_phy_list = [Tmin, Tmax, Prcp, Pet]
    x_phy = torch.stack(x_phy_list, dim=1) # Shape: [time, vars]
    
    # c. 扩展以匹配 [time, grids, vars] 的形状
    x_phy = x_phy.unsqueeze(1).repeat(1, n_grid, 1)

    # d. 准备流域面积和其他静态输入
    watershed_area = torch.tensor([500.0] * n_grid, device=device) # Shape: [grids]

    # e. 创建最终的输入字典 x_dict
    x_dict = {'x_phy': x_phy, 'area': watershed_area}
