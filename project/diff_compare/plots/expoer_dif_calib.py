...
OUTPUT_TEST_CSV_NAME = (
    f"{os.path.dirname(os.path.abspath(__file__))}/csv/dif_test_kge.csv"
)
OUTPUT_TRAIN_CSV_NAME = (
    f"{os.path.dirname(os.path.abspath(__file__))}/csv/dif_train_kge.csv"
)
...
# 默认参数
LOSS_NAME = "KgeLoss"  # 读取 KgeLoss 文件夹
METRIC_KEY = "kge"  # 读取 json 中的 "kge" 键
...
def get_model_results(model_name: str):
    ...
    # 读取 (559, 16)
    train_data = _load_json_file(train_path, METRIC_KEY, N_BASINS, 16)
    test_data = _load_json_file(test_path, METRIC_KEY, N_BASINS, 16)
