...
def mopex_step(
    ...
    # Physics: Melt logic (no delta_t; assume dt=1)
    melt_pot = F.relu(T - tcrit) * ddf
    flux_qn = torch.minimum(melt_pot, Sn)  # Actual melt
    ...
    # Surface Runoff (Saturation excess) inlined
    flux_q1f = F.relu(S1 - Sb1)
    S1 = S1 - flux_q1f

    # Percolation to S2 (recharge inlined)
    flux_qw_pot = S1 / (tw + 1e-6)
    flux_qw = torch.minimum(flux_qw_pot, S1)
    S1 = S1 - flux_qw

    # Evaporation (Using PET_effective) (evap inlined, dt=1)
    flux_et1_pot = PET_effective * (S1 / (Sb1 + 1e-6))
    ...
    # Calculate Potential Overflow (if mechanism is fully active)
    flux_q2f_pot = F.relu(S2 - Sb2)

    # Apply Weight: Only w_sub fraction flows out rapidly
    flux_q2f = flux_q2f_pot * w_sub
    S2 = S2 - flux_q2f  # Remove actual outflow

    # Baseflow (Slow drainage from remaining S2) (inlined)
    flux_q2u_pot = S2 / (tu + 1e-6)
    flux_q2u = torch.minimum(flux_q2u_pot, S2)
    S2 = S2 - flux_q2u

    # Evaporation from S2 (inlined)
    flux_et2_pot = PET_effective * (S2 / (Se + 1e-6))
    ...
    flux_qf = torch.minimum(Sc1 / (tc + 1e-6), Sc1)
    Sc1_new = torch.clamp(Sc1 - flux_qf, min=0.0)

    Sc2 = Sc2 + flux_q2u
    flux_qs = torch.minimum(Sc2 / (tc + 1e-6), Sc2)
