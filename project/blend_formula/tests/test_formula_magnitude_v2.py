...
用法:
    python project/blend_formula/tests/test_formula_magnitude_v2.py
"""
...
# ===================================================================
# 快速流 / 基流 候选公式 (本地定义, 待验证后再合并至模型)
# 核心改动: 所有公式共享 k 作为基准速率, 用 ratio 函数调制
# ===================================================================
...
def base_power(S, k, n, Smax, eps=1e-6):
    """幂律水库: k_eff = k * ratio^(n-1)
    n>1: 蓄满时排泄加速; n<1: 蓄满时排泄减缓."""
    ratio = (S / (Smax + eps)).clamp(eps, 1.0)
    return S * (1.0 - torch.exp(-k * ratio.pow(n - 1.0)))
...
def print_table(title: str, rows: list[dict]):
    ...
    valid_means = [m for m in means if m > 1e-3]  # 过滤 < 1mm/day 的值
    ...
    if len(valid_means) > 1:
        ...
        print(f"  ℹ️  只有一个公式输出 > 1e-3，无法比较")
    else:
        print(f"  ℹ️  所有输出接近 0 (物理上合理)，跳过量级检查")
...
BOUNDS_ORIGINAL = {
    ...
    "x4":  [-5.0, -1.0],  "x5":  [0.001, 0.3],   "x6":  [0.3, 1.5],
    "x7":  [0.5, 3.0],    "x8":  [0.0, 3.0],     "x9":  [0.0, 0.05],
...
BOUNDS_ADJUSTED = {
    "x1":  [0.0, 1.0],    "x2":  [0.3, 3.0],    "x3":  [0.5, 3.0],
    "x4":  [-5.0, -1.0],  "x5":  [0.001, 0.3],   "x6":  [0.3, 1.5],  # ✓ n_quick
    "x7":  [0.5, 3.0],    "x8":  [0.0, 3.0],     "x9":  [0.0, 0.05], # ✓ lambda
    "x10": [0.0, 0.45],   "x11": [-5.0, -2.0],   "x12": [0.5, 2.0],
    "x13": [0.0, 0.1],    "x14": [0.01, 0.3],    "x15": [0.005, 0.1],
    "x16": [-5.0, 2.0],   "x17": [0.3, 1.0],     "x18": [0.0, 5.0],
    "x19": [0.0, 0.4],    "x24": [1.5, 3.0],     "x25": [0.0, 5.0],
...
# 选择使用哪个参数范围
USE_ADJUSTED = True
BOUNDS = BOUNDS_ADJUSTED if USE_ADJUSTED else BOUNDS_ORIGINAL
...
def test_quickflow(S_ratio=0.5):
    """快速流: 3个公式 (共享 k_quick 作为基准速率)"""
    Smax = sample_uniform(50, 500)
    S = Smax * S_ratio
    k_quick = 10.0 ** p("x4")  # log_k_quick -> k_quick

    q1 = quick_linear(S, k_quick)
    q2 = quick_vic(S, Smax, k_quick, p("x6"))
    q3 = quick_topmodel(S, Smax, k_quick, p("x7"))
...
def main():
    print("候选公式输出量级对比测试 (改进版)")
    print(f"样本数 B={B}")
    print(f"参数范围: {'调整后' if USE_ADJUSTED else '原始'}")
    if USE_ADJUSTED:
        print("  - x5 (q_max): [0, 100] -> [0, 20]")
        print("  - x7 (topmodel_lambda): [5, 10] -> [2, 6]")
        print("  - x11 (log_k_base): [-5, -2] -> [-4, -2.5]")
        print("  - x12 (n_base): [0.5, 2.0] -> [0.5, 1.5]")
    print()
