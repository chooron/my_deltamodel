...
def hbv_step_split(
    ...
    eps = 1e-5
    max_snow = 1000.0  # 最大积雪量 (mm)
    max_melt = 500.0   # 最大融雪水量 (mm)
    max_sm = 1000.0    # 最大土壤水分 (mm)
    max_suz = 500.0    # 最大上层地下水 (mm)
    max_slz = 1000.0   # 最大下层地下水 (mm)
    
    # 使用 clamp 确保状态变量在物理合理范围内
    # 注意：这里使用 clamp 而不是 detach，以保持梯度流
    snow_out = torch.clamp(snow_out, min=eps, max=max_snow)
    melt_out = torch.clamp(melt_out, min=eps, max=max_melt)
    sm_after = torch.clamp(sm_after, min=eps, max=max_sm)
    suz_out = torch.clamp(suz_out, min=eps, max=max_suz)
    slz_out = torch.clamp(slz_out, min=eps, max=max_slz)
