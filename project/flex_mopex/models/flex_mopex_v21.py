...
class FlexMopexV21(nn.Module):
    """MOPEX 水文模型：与MultiHeadNetWithDynamicWeights兼容版本 (Sigmoid版本)

    版本二_1：使用sigmoid激活函数替代softmax
    - 接收来自MultiHeadNetWithDynamicWeights的动态权重logits
    - 权重格式: [n_steps, n_grid, 4] (不再是4x2，直接输出4个权重)
    - 参数格式: {"params": [B, 12*nmul], "gamma_uh": [B, 2], "weights": [n_steps, B, 4]}
    - 使用sigmoid激活函数，每个权重独立在[0,1]范围内
    """
    ...
    def unpack_parameters(
        ...
        Args:
            parameters: 字典，包含 MOPEX 参数、路由参数和动态权重
                例如: {
                    "params": [B, 12*nmul],
                    "gamma_uh": [B, 2],
                    "weights": [n_steps, B, 4]  # 动态权重logits (sigmoid版本)
                }

        Returns:
            - mopex_params: MOPEX 模型参数 [B, 12, nmul]
            - routing_params: 路由参数 [B, 2]
            - weights_logits: 动态权重logits [n_steps, B, 4]
        """
        ...
        # 提取动态权重logits（不需要激活，后续会用sigmoid）
        weights_logits = parameters["weights"]  # [n_steps, B, 4]
    ...
    def _process_dynamic_weights(
        ...
        """
        处理动态权重logits，转换为权重激活概率 (Sigmoid版本)

        Args:
            weights_logits: [n_steps, n_grid, 4] 权重logits

        Returns:
            weights_on: [n_steps, n_grid, 4] 每个时间步的权重激活概率
        """
        # 应用sigmoid激活函数，每个权重独立在[0,1]范围内
        weights_on = torch.sigmoid(weights_logits)  # [n_steps, n_grid, 4]
