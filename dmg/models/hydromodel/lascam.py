...
LASCAM_PARAMS_BOUNDS = {
    ...
    "bf": [0.0, 5.0],         # Catchment-scale infiltration non-linearity parameter [-]
...
def lascam_step(
    ...
    flux_qse = torch.clamp(flux_qse, min=0.0, max=flux_pg)
    ...
    flux_pc = torch.clamp(flux_pc, min=0.0, max=rem_pg)
    ...
    limit_ratio = torch.clamp((1.0 - tmp_phiss) / (1.0 - tmp_phic + nearzero), min=0.0, max=1.0)
    ...
    S3_tmp = S3 - flux_qb # Start with b-flow because it's in multiple stores' ODEs
    ...
    flux_qa = saturation_11(aa, ba, S2_tmp2, amin, amax, torch.ones_like(P), nearzero=nearzero)
