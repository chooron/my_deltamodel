...
# Parameter range dictionary (based on MARRMoT m_31_mopex3_8p_5s)
MOPEX3_PARAMS_BOUNDS = {
    ...
    "s3max": [1.0, 2000.0],   # Maximum groundwater/root zone storage [mm]
...
MOPEX3_PARAMS_DESC = {
    ...
    "s3max": "Maximum groundwater/root zone storage [mm]",
...
def create_initial_state(
    ...
    """
    Create initial states for MOPEX-3 model.
    S1: Snow store
    S2: Surface soil moisture store
    S3: Groundwater/Root zone store
...
def mopex3_step(
    ...
    T: torch.Tensor,
    PET: torch.Tensor,
    # Parameters matching MOPEX3_PARAMS_BOUNDS keys
    tcrit: torch.Tensor,
    ddf: torch.Tensor,
    s2max: torch.Tensor,
    tw: torch.Tensor,
    ...
    """
    MOPEX-3 model single-step calculation.
    ...
    # flux_q1f: saturation excess runoff
    ...
    # --- 3. Groundwater/Root Zone Store Process (S3) ---
    ...
    # Remaining PET after S2 ET
    PET_rem = F.relu(PET - flux_et1)
    
    # flux_et2: Actual evaporation from S3
    # Use se_frac * s3max as the reference storage for ET in S3
    ...
    # flux_q2u: Slow flow routing response (to S5)
    ...
    # Inflow is fast saturation excess from S2 and overflow from S3
    ...
    # Inflow is q2u from S3
    S5_tmp = S5 + flux_q2u
    S5_tmp = torch.clamp(S5_tmp, min=nearzero)
    
    # flux_qs: Slow runoff discharge
