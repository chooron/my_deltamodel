...
# 引入通量计算函数
from dmg.models.hydromodel.flux.evap import evap_12
from dmg.models.hydromodel.flux.saturation import saturation_5
from dmg.models.hydromodel.flux.split import split_1

# 引入单位线
# 1. Exponential Decay UH (for fast/slow routing) -> uh_5_half
from dmg.models.hydromodel.unithydro.uh_exp_5 import DplExp5

# 2. Pure Delay UH (for total flow) -> uh_8_delay
from dmg.models.hydromodel.unithydro.uh_delay_8 import DplDelay8
...
_ihacres_production_step = torch.compile(
    _ihacres_production_step_impl
)
...
class Ihacres(UnifyV2):
    ...
    def _run_model(
        ...
        for t in range(n_steps):
            flux_uq, flux_us, flux_ea, S1 = _ihacres_production_step(
        ...
        # ==========================================================
        # Phase 3: Serial Convolution (Total Delay)
        # ==========================================================
        # Apply the pure delay (tau_d) to the combined flow
        # This is the "route(flux_xq + flux_xs, uh_t)" part from MATLAB
