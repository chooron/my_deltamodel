...
def phenology_1(T: torch.Tensor, p1: torch.Tensor, p2: torch.Tensor, Ep: torch.Tensor, nearzero: float = 1e-6) -> torch.Tensor:
    """
    ...
    scale = torch.minimum(
        torch.ones_like(T), 
        F.relu((T - p1) / (p2 - p1 + nearzero))
    )
    return scale * Ep

def phenology_2(p1: torch.Tensor, p2: torch.Tensor, p3: torch.Tensor, t: torch.Tensor, tmax: torch.Tensor, nearzero: float = 1e-6) -> torch.Tensor:
    """
